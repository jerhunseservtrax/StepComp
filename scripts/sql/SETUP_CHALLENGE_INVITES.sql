-- ============================================================
-- CHALLENGE INVITES SYSTEM SETUP
-- ============================================================
-- Run this in Supabase Dashboard → SQL Editor
-- ============================================================

-- Check if tables exist (for informational purposes)
SELECT 
    CASE 
        WHEN EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'challenge_invites') 
        THEN '✅ challenge_invites table exists'
        ELSE '❌ challenge_invites table MISSING'
    END as challenge_invites_status,
    CASE 
        WHEN EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'inbox_notifications') 
        THEN '✅ inbox_notifications table exists'
        ELSE '❌ inbox_notifications table MISSING (will use notifications table)'
    END as inbox_status,
    CASE 
        WHEN EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'notifications') 
...
-- ============================================================
-- 2. Notifications Table (if it doesn't exist)
-- ============================================================
...
SELECT 
    '✅ Setup complete! Run this query to verify:' as status;

SELECT 
    'challenge_invites' as table_name,
    COUNT(*) as row_count
FROM public.challenge_invites
UNION ALL
SELECT 
