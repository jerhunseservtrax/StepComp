...
struct ExerciseHistoryStat: Identifiable {
    let exerciseName: String
    let sessionCount: Int
    let totalVolume: Double
    let maxWeight: Double
    let totalSets: Int
    let volumeOverTime: [(date: Date, volume: Double)]

    var id: String { exerciseName }

    var sparklineValues: [Double] {
        volumeOverTime.map(\.volume)
    }
}
...
final class MetricsViewModel: ObservableObject {
    ...
    @Published var exerciseHistoryStats: [ExerciseHistoryStat] = []
    ...
    func addBodyMetric(bodyFatPercent: Double?, waistCm: Double?) {
        ...
        let expectedUserId = AuthService.shared.currentUser?.id
        Task.detached(priority: .utility) {
            await MetricsService.shared.syncBodyMetric(bodyFatPercent: bodyFatPercent, waistCm: waistCm, expectedUserId: expectedUserId)
        }
    }
    ...
    private func loadPillarMetrics(healthKitService: HealthKitService) async {
        ...
        exerciseHistoryStats = computeExerciseHistory(sessions: sessions)
    ...
    private func computeExerciseHistory(sessions: [CompletedWorkoutSession]) -> [ExerciseHistoryStat] {
        struct Accumulator {
            ...
            var volumeBySession: [(date: Date, volume: Double)] = []
        }
        ...
        for session in sessions {
            ...
            for workoutExercise in session.exercises {
                let key = workoutExercise.exercise.name.lowercased()
                if map[key] == nil {
                    map[key] = Accumulator(displayName: workoutExercise.exercise.name)
                }
                ...
                for set in workoutExercise.sets where set.isCompleted {
                    completedSetCount += 1
                    if let effectiveWeight = set.effectiveWeightForVolume {
                        sessionMaxWeight = max(sessionMaxWeight, effectiveWeight)
                        if let reps = set.reps {
                            sessionVolume += effectiveWeight * Double(reps)
                        }
                    }
                }
                ...
                map[key]!.totalVolume += sessionVolume
                map[key]!.maxWeight = max(map[key]!.maxWeight, sessionMaxWeight)
                map[key]!.totalSets += completedSetCount
                map[key]!.sessionDates.insert(sessionDay)
                map[key]!.volumeBySession.append((date: session.endTime, volume: sessionVolume))
            }
        }

        return map.values
            .map { acc in
                let sorted = acc.volumeBySession.sorted { $0.date < $1.date }
                return ExerciseHistoryStat(
                    exerciseName: acc.displayName,
                    sessionCount: acc.sessionDates.count,
                    totalVolume: acc.totalVolume,
                    maxWeight: acc.maxWeight,
                    totalSets: acc.totalSets,
                    volumeOverTime: sorted
                )
            }
