...
struct WorkoutProgressView: View {
    ...
    @State private var bodyWeightDisplay: Int = 0
    ...
    private var estimated1RMSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("ESTIMATED 1RM")
                ...
                .foregroundColor(FitCompColors.textSecondary.opacity(0.6))
                .tracking(1.2)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ZStack {
                        Circle()
                            .fill(FitCompColors.buttonTextOnPrimary.opacity(0.15))
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("BODY WEIGHT")
                            ...
                            .foregroundColor(FitCompColors.buttonTextOnPrimary.opacity(0.4))
                            .tracking(0.5)
                        
                        HStack(alignment: .firstTextBaseline, spacing: 2) {
                            Text("\(bodyWeightDisplay)")
                ...
                .cornerRadius(24)
                .shadow(color: FitCompColors.primary.opacity(0.15), radius: 12, x: 0, y: 6)
    ...
    private func refreshBodyWeightAndSteps() {
        ...
        if storedKg > 0 {
            bodyWeightDisplay = unitManager.weightFromStorage(storedKg)
        } else {
            Task { @MainActor in
                if let kg = try? await healthKitService.getWeight() {
                    bodyWeightDisplay = unitManager.weightFromStorage(Int(kg))
                }
            }
        }
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                ...
                Text("LAST 90 DAYS")
                    ...
                    .foregroundColor(FitCompColors.textSecondary.opacity(0.4))
            }
                        ...
                        ForEach(Array(week.enumerated()), id: \.offset) { dayIndex, date in
                            if let date = date {
                                ...
                                RoundedRectangle(cornerRadius: 3)
                                    .fill(hasWorkout ? FitCompColors.primary : FitCompColors.textSecondary.opacity(0.15))
