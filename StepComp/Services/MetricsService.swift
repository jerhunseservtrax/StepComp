...
final class MetricsService: ObservableObject {
    ...
    func fetchMetricsSummary(days: Int = 30) async -> MetricsSummary? {
        #if canImport(Supabase)
        guard let cacheKey = await currentUserScopedCacheKey("metrics_summary_\(days)") else {
            return nil
        }
    ...
    func fetchWeightHistory(days: Int = 90) async -> [WeightHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheKey = await currentUserScopedCacheKey("weight_history_\(days)") else {
            return []
        }
    ...
    func fetchWorkoutHistory(days: Int = 90) async -> [WorkoutHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheKey = await currentUserScopedCacheKey("workout_history_\(days)") else {
            return []
        }
        return await OfflineCacheService.fetchArrayWithFallback(key: cacheKey) {
    ...
    #if canImport(Supabase)
    private func currentUserScopedCacheKey(_ key: String) async -> String? {
        do {
            let session = try await supabase.auth.session
            return OfflineCacheService.scopedKey(key, userId: session.user.id.uuidString)
        } catch {
            return nil
        }
    }
