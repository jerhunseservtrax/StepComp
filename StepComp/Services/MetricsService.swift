...
final class MetricsService: ObservableObject {
    ...
    func fetchMetricsSummary(days: Int = 30) async -> MetricsSummary? {
        #if canImport(Supabase)
        guard let cacheUserId = personalCacheUserId() else { return nil }
        return await OfflineCacheService.fetchWithFallback(key: "metrics_summary_\(days)", userId: cacheUserId) {
    ...
    func fetchWeightHistory(days: Int = 90) async -> [WeightHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheUserId = personalCacheUserId() else { return [] }
        return await OfflineCacheService.fetchArrayWithFallback(key: "weight_history_\(days)", userId: cacheUserId) {
    ...
    func fetchWorkoutHistory(days: Int = 90) async -> [WorkoutHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheUserId = personalCacheUserId() else { return [] }
        return await OfflineCacheService.fetchArrayWithFallback(key: "workout_history_\(days)", userId: cacheUserId) {
    ...
    private func personalCacheUserId() -> String? {
        guard let userId = AuthService.shared.currentUser?.id.trimmingCharacters(in: .whitespacesAndNewlines),
              !userId.isEmpty else {
            return nil
        }
        return userId
