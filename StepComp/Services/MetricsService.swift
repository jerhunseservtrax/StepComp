...
final class MetricsService: ObservableObject {
    ...
    func fetchMetricsSummary(days: Int = 30) async -> MetricsSummary? {
        #if canImport(Supabase)
        guard let cacheUserId = await authenticatedCacheUserId() else { return nil }
    ...
    func fetchWeightHistory(days: Int = 90) async -> [WeightHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheUserId = await authenticatedCacheUserId() else { return [] }
    ...
    #if canImport(Supabase)
    private func authenticatedCacheUserId() async -> String? {
