...
final class MetricsService: ObservableObject {
    ...
    func fetchMetricsSummary(days: Int = 30) async -> MetricsSummary? {
        #if canImport(Supabase)
        guard let cacheScope = await currentCacheScope() else { return nil }
        return await OfflineCacheService.fetchWithFallback(key: "metrics_summary_\(days)", scope: cacheScope) {
    ...
    func fetchWeightHistory(days: Int = 90) async -> [WeightHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheScope = await currentCacheScope() else { return [] }
        return await OfflineCacheService.fetchArrayWithFallback(key: "weight_history_\(days)", scope: cacheScope) {
    ...
    func fetchWorkoutHistory(days: Int = 90) async -> [WorkoutHistoryPoint] {
        #if canImport(Supabase)
        guard let cacheScope = await currentCacheScope() else { return [] }
        return await OfflineCacheService.fetchArrayWithFallback(key: "workout_history_\(days)", scope: cacheScope) {
