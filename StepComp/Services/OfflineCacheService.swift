...
enum OfflineCacheService {
    ...
    static func save<T: Encodable>(_ value: T, key: String, userId: String?) {
        guard let scopedKey = scopedKey(key, userId: userId) else { return }
        let url = cacheDirectory.appendingPathComponent(safeName(scopedKey) + ".json")
        do {
    ...
    static func load<T: Decodable>(_ type: T.Type, key: String, userId: String?) -> T? {
        guard let scopedKey = scopedKey(key, userId: userId) else { return nil }
        let url = cacheDirectory.appendingPathComponent(safeName(scopedKey) + ".json")
    ...
    static func fetchWithFallback<T: Codable>(
        key: String,
        userId: String?,
        isUserStillActive: () async -> Bool,
        fetch: () async throws -> T
    ) async -> T? {
        do {
            let value = try await fetch()
            guard await isUserStillActive() else { return nil }
    ...
    static func fetchArrayWithFallback<T: Codable>(
        ...
        do {
            let value = try await fetch()
            guard await isUserStillActive() else { return [] }
            save(value, key: key, userId: userId)
            return value
        } catch {
            #if DEBUG
