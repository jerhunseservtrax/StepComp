...
| Offline Cache | `OfflineCacheService.swift`, `MetricsService.swift`, `AuthService.swift` | Generic disk-backed Codable cache with fetch-with-fallback for offline resilience; user metrics cache keys are scoped by authenticated user and cleared on sign-out |
