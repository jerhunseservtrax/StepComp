...
| Offline Cache | `OfflineCacheService.swift`, `AuthService.swift` | Generic disk-backed Codable cache with fetch-with-fallback for offline resilience; cleared whenever signed-out state is applied to prevent cross-account fallback data exposure |
