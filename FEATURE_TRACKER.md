...
| Feature | File(s) | Description |
|---------|---------|-------------|
| Supabase Auth | `AuthService.swift` | Singleton auth with Keychain persistence, Apple Sign In, Google OAuth URL generation, auth state listener |
...
| Sensitive URL Redaction | `SensitiveURLRedactor.swift` | Redacts OAuth access, refresh, ID, provider, and authorization code values before debug logging URLs |
