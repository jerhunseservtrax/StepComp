...
- **Fix:** Added user-scoped cache APIs and moved metrics/leaderboard fallbacks onto current-user keys. Sign-out now clears offline cache plus in-memory and persisted challenge fallback state immediately, even if the Supabase signed-out event is delayed or sign-out fails locally. Active workout cleanup now cancels the in-memory session as well as persisted/widget/live-activity state. In-flight profile and cache responses are guarded so late completions cannot repopulate private state after sign-out/account switch.
