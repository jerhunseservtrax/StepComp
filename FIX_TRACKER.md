...
- **Fix:** Added user-scoped offline cache keys for metrics and leaderboard data. Sign-out now clears existing offline cache files, challenge service in-memory/local caches, local workout/weight stores, and metrics sync bookkeeping so legacy unscoped entries cannot be replayed or synced into another account.
- **Files:** `OfflineCacheService.swift`, `MetricsService.swift`, `ChallengeService.swift`, `AuthService.swift`, `WorkoutViewModel.swift`, `WeightViewModel.swift`
