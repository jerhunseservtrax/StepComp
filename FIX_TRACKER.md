...
- **Fix:** Added user-scoped cache APIs and moved metrics/leaderboard fallbacks onto current-user keys. Sign-out now clears offline cache; in-memory and persisted challenge fallback state; workout templates/history; weight entries; food logs/photos; body metrics/nutrition logs; transformation photos; and profile/nutrition defaults immediately, even if the Supabase signed-out event is delayed or sign-out fails locally. Active workout cleanup now cancels the in-memory session as well as persisted/widget/live-activity state. In-flight profile, cache, challenge, and metrics sync responses are guarded so late completions cannot repopulate or upload private state after sign-out/account switch.
